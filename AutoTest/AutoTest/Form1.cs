...
namespace AutoTest
{
    public partial class Form1 : Form
    {
        ...
        */
            ...
            if (data_to_read > 0)
            {
                ...
                foreach (string s in textArray)
                {
                    if (s.Length > 4 && s.Contains("Cmd:"))
                    {
                        ...
                        if (Global.keyCodeDic.ContainsKey(temp) && clickFlag == true)
                        {
                            if (isFirsttime)
                            {
                                isFirsttime = false;
                                lastKey = Global.keyCodeDic[temp];
                                //Console.WriteLine(Global.keyCodeDic[temp] );
                            ...
                            else
                            {
                                ...
                                DataGridView_Schedule.Rows[DataGridView_Schedule.RowCount - 2].Cells[9].Value = delayTime;
                            }
        ...
        public long calTime()
        {
            ...
            {
                st1.Stop();//停止st1
                costTime = st1.ElapsedMilliseconds; //回傳st1的時間
                st1.Reset();
                st1IsStart = false;
                st2.Start();//啟動st2
                st2IsStart = true;
        ...
        {
            StreamReader str = new StreamReader(keyPath);
            string item = string.Empty;
            while ((item = str.ReadLine()) != null)
            {
                if (item.IndexOf("Cmd") != -1)
                {
                    //Global.keyCodeDic.Add(item.Substring(item.IndexOf("raw") + 4, item.IndexOf("=") - (item.IndexOf("raw") + 4)).Trim(),
                    // item.Substring(item.IndexOf("=") + 1, item.Length - (item.IndexOf("=") + 1)).Trim());  //Use raw as key
                    Global.keyCodeDic.Add(item.Substring(item.IndexOf("Cmd") + 4, item.IndexOf("=") - (item.IndexOf("Cmd") + 4)).Trim(),
                         item.Substring(item.IndexOf("=") + 1, item.Length - (item.IndexOf("=") + 1)).Trim());
                }
            }
            str.Close();
    ...
    public class Global//全域變數//
    {
        ...
        public static Dictionary<string, string> keyCodeDic = new Dictionary<string, string>();
